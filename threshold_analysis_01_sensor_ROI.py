...
if __name__ == "__main__":
	...
	for subject in subjectIDs:
		...
		# load in TFR object. one per trial per subject
		homeDir = os.path.expanduser("~")
		dataDir = homeDir + '/camcan/'


		trial_tfrs = []
		for trial in range(63):
			
			TFRfname = dataDir + subject + '_left_trial=' + str(trial) + '_frange=5-105Hz_fstep=1Hz-tfr.h5'

			if os.path.exists(TFRfname):

				# import AverageTFR object
				TFR_object = mne.time_frequency.read_tfrs(TFRfname)[0]
				TFR_object.crop(fmin=fmin, fmax=fmax, tmin=tmin, tmax=tmax) 
				TFR = TFR_object.data # shape= [1x16x1001]
				TFR = np.squeeze(TFR, axis=0)
				trial_tfrs.append(TFR)
	...
	df_plot.to_csv(dataDir + 'ROI_sensor_shin_threshold.csv')
